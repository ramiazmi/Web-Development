class SectorsController < ApplicationController
  ...
  # GET /sectors.json
  def index
    @sectors = Sector.search(params[:search]).order(:id)
  end
  ...
  def create
    ...
    respond_to do |format|
      if @sector.save
        format.html { redirect_to sectors_url, notice: 'تم الحذف بنجاح' }
  ...
  def update
    respond_to do |format|
      if @sector.update(sector_params)
        format.html { redirect_to sectors_url, notice: 'تم التعديل بنجاح' }
  ...
  private
    ...
    def sector_params
      params.require(:sector).permit(:programme, :percentage, :is_active)
    end
end
